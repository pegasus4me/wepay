...
async function runDemo() {
    ...
    try {
        ...
        if ((receipt as any).sponsorship) {
            ...
            console.log(`\n[Weppo Abstraction] 🛡️ This transaction was fully sponsored!`);
            console.log(`[Weppo Abstraction] Gas used: ${sp.gasUsed}`);
            console.log(`[Weppo Abstraction] Cumulative platform sponsorship for this agent: ${sp.cumulativeAgentSponsorship} ETH`);
        }

        // 5. Final balance
        const finalBalance = await weppo.getBalance();
